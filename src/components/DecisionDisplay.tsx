...
const DecisionDisplay: React.FC<DecisionProps> = ({ decision }) => {
  ...
  const formatDecisionText = (text: string) => {
    ...
    const summaryMatch = text.match(
      /I\.\s*SUMMARY OF DISPUTE\s*([\s\S]*?)(?=II\.|$)/
    );
    const factsMatch = text.match(
      /II\.\s*ESTABLISHED FACTS\s*([\s\S]*?)(?=III\.|$)/
    );
    const evidenceMatch = text.match(
      /III\.\s*EVIDENCE CONSIDERED\s*([\s\S]*?)(?=IV\.|$)/
    );
    const rulesMatch = text.match(
      /IV\.\s*APPLICABLE RULES\s*([\s\S]*?)(?=V\.|$)/
    );
    const reasoningMatch = text.match(
      /V\.\s*TRIBUNAL REASONING\s*([\s\S]*?)(?=VI\.|$)/
    );
    const remedyMatch = text.match(
      /VI\.\s*RULING AND REMEDY\s*([\s\S]*?)(?=VII\.|$)/
    );
    const notesMatch = text.match(
      /VII\.\s*ADDITIONAL NOTES\s*([\s\S]*?)(?=---|$)/
    );
    ...
    if (factsMatch) {
      ...
      const afterFinds = factsContent.match(
        /Based on the evidence provided, the Tribunal finds that:\s*([\s\S]*)/
      );
    ...
    if (evidenceMatch) {
      ...
      const afterAssessed = evidenceContent.match(
        /The Tribunal assessed, inter alia:\s*([\s\S]*)/
      );
      sections.evidence = afterAssessed
        ? afterAssessed[1].trim()
        : evidenceContent;
    }
    if (rulesMatch) {
      ...
      const afterGoverned = rulesContent.match(
        /This dispute is governed by the following provisions of the AI Judge™ Rules of Procedure:\s*([\s\S]*)/
      );
  ...
  const isLowConfidence = decision.decision.confidence_score < 0.6;
      ...
      <div className="decision-display">
        <div className="decision-header">
          ...
          <div className="case-info">
            ...
            <p>
              Date:{" "}
              {sections.timestamp
                ? new Date(sections.timestamp).toLocaleDateString("en-US", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  })
          ...
          <section className="decision-section">
            ...
            <div className="facts-list">
              {sections.facts
                ...
                .filter((f) => f.trim())
                .map((fact, index) => (
                  <p key={index} className="fact-item">
                    • {fact.trim()}
                  </p>
                ))}
            </div>
          ...
          <section className="decision-section">
            ...
            <div className="evidence-list">
              {sections.evidence
                ...
                .filter((e) => e.trim())
                .map((evidence, index) => (
                  <p key={index} className="evidence-item">
                    • {evidence.trim()}
                  </p>
                ))}
            </div>
          ...
          <section className="decision-section ruling">
            ...
            <div className="ruling-box">
              ...
              <div className="remedy-details">
                <p>
                  <strong>Remedy Type:</strong>{" "}
                  {decision.decision.remedy_awarded.type
                    .replace(/_/g, " ")
                    .toUpperCase()}
                </p>
                {decision.decision.remedy_awarded.amount_usd > 0 && (
                  <p>
                    ...
                    {decision.decision.remedy_awarded.amount_usd.toFixed(2)}
                  </p>
                )}
                <p>
                  <strong>Compliance Deadline:</strong>{" "}
                  {new Date(
                    decision.decision.compliance_deadline
              ...
              <div className="confidence-info">
                ...
                {isLowConfidence && (
                  <div className="appeal-notice">
                    ...
                    <p>
                      This decision may be appealed within 5 days pursuant to
                      Article 6.4 of the AI Judge™ Rules of Procedure.
                    </p>
                  </div>
                )}
              </div>
        ...
        <div className="decision-footer">
          ...
          <div className="footer-text">
            <p>
              This decision is final and binding on all parties, subject only to
              appeal rights as stated above.
            </p>
